...
def lectura_entradas (archivo_entradas): #Crear la función lectura_entradas con parámetros de entrada: nombre del archivo.txt con la información de las entradas.
    ...
    l_k=[] # Crear una lista vacia para el número de clases de cada set de datos.
...
def medidas (data): # Crear la función medidas con parámetros de entrada: lista de datos.
    ...
    r_desviacion= statistics.stdev(data) # Calcular la desviación estandar
...
def clases (data, k, r_rango): # Crear la función medidas con parámetros de entrada: lista de datos.
    r_clases=[] # Crear lista vacía para almacenar las clases.
    
    n_decimales=len(str(data[0]))-str(data[0]).index(".") # Calcular el número de decimales de la ventana.
    #   n_decimales=(cantidad de decimales de los datos) + 1
    #   n_decimales= (tamaño de la cadena de un dato) - (índice del elemento.)
    ventana=(1/(10**n_decimales)) # Calcular la ventana para el tamaño de clases.
    #   ventana= margen de +/- 0.0..01 (para asegurar que las clases cubran todos los rangos, debe tener un decimal mas que los datos de entrada)
    tamaño=(r_rango+(ventana*2))/k # Calcular el tamaño de las clases.
    l_inferior=min(data)-(ventana/2) # Determinar el primer límite inferior.

    c=0 #Iniciar contador.
    while c<k: # Mientras el contador sea menor que k haga:
        l_superior=l_inferior+tamaño # Límite superior = límite inferior + tamaño de clase
        r_clases.append((round(l_inferior,n_decimales), round(l_superior,n_decimales))) # Añadir a la lista de clases una tupla con los dos límites.
        # Redondear los límites a decimales originales + 1.
        l_inferior=l_superior # Convertir el nuevo límite inferior en el límite superior actual.
        c+=1

    return r_clases # Devolver la lista de las clases.
...
def resultados (r_clases,tabla,r_medidas,archivo): # Crear la función frecuencias con parámetros de entrada: lista de clases, lista de datos y archivo de datos.
    ...
    if os.path.exists("Resultados")==False: # Si la carpeta "Resultados" No esxiste:
    ...
    hoja_2.set_column('A:A', ancho) # Definir el tamaño de la culumana A de la hoja 2.
...
def histograma (data,r_clases,k,fa,archivo,color): # Crear la función frecuencias con parámetros de entrada: lista de clases, lista de datos, archivo de datos y color del gráfico.
    ...
    #   Malla principal para el eje y, con color gris claro, y estílo de línea punteada.
...
def hist_comparativo (l_data,l_k,l_colores,l_archivos): # Crear la función frecuencias con parámetros de entrada: lista de clases, lista de datos, archivo de datos y color del gráfico.
    
    raiz=os.getcwd() # Definir la ruta actual
    os.chdir("Resultados") # Cambiar del directorio actual a la carpeta de resultados
    ...
    plt.savefig('Histograma comparativo.png') # Guardar el gráfico.
    plt.show(block=False) # Mostrar el grafico, sin interrumpir el programa
    plt.close() # Cerrar el gráfico
...
def main():
    ...
    for i in range(len(l_archivos)):
        ...
        m_centramiento= medidas (datos) # Calcular y almacenar el resultados de las medidas.
        rango=m_centramiento[4] # Extraer el rango
        
        "3. Crear la tabla de frecuencias" 
        "3.1. Definir las clases"
        intervalos=clases(datos,n_k,rango) # Calcular y almacenar las clases.
        "3.2. Calcular las frecuencias"
        t_frecuencias=frecuencias (intervalos,datos) # Calcular y almacenar las frecuencias.
        f_absoluta=t_frecuencias[0] # Extraer la frecuencia abosulta 
    ...
    if len(l_archivos)>1: # Si hay más de un archivo de datos
        hist_comparativo(l_datos,l_k,l_colores,l_archivos)# Crear, mostrar y guardar histograma comparativo.
